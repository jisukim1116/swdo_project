...
$(function(){
	$(document).ready(function(){
		...
		var cnt_upt = $('#cnt_upt').val();
		
		if(cnt_upt == 1){
			alert("회원 정보 갱신에 성공하였습니다.")
		}
		if(cnt_upt == 0){
			alert("회원 정보 갱신에 실패하였습니다. 다시 시도해주세요.")
		}		
	})
});
...
<section class="row" style="display: flex;justify-content: center;">
	<div id="grid" class="">
		...
		<form action="/user/update" method="post" onsubmit="return formCheck();">
			<div id="grid">
				<input type="text" class="" style="border-bottom: 1px solid; " id="user_name" name="user_name" value="${detail.user_name }">
			</div>
			<div id="grid">
				<input type="text" class="" style="border-bottom: 1px solid; " id="user_id" name="user_id" value="${detail.user_id }" readonly="readonly">
			</div>
			<div id="grid">
			...
			<div id="grid" style="display: flex">
				<button type="submit" class="" style="width:50%;background-color: black;color:white; "><small>수정</small></button>
				<input type="button" class="" style="width:50%;font-size: x-small" value="회원 탈퇴" onclick="userDelete('${detail.user_id }')" >
			</div>
		</form>
	</div>
</section>
